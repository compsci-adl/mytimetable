...
describe('parseRenovatePrBody & parsePrBody', () => {
	it('should parse digest updates from Renovate PR body tables (user example)', () => {
		const body = `
This PR contains the following updates:

| Package | Type | Update | Change |
|---|---|---|---|
| [actions/checkout](https://redirect.github.com/actions/checkout) | action | pinDigest |  → \`d23441a\` |
| [actions/github-script](https://redirect.github.com/actions/github-script) | action | pinDigest |  → \`f28e40c\` |
...
`;
		...
		expect(changes).toEqual([
			'bump actions/checkout to d23441a',
			'bump actions/github-script to f28e40c',
		]);
	});

	it('should parse version range updates from Renovate PR tables', () => {
		const body = `
This PR contains the following updates:

| Package | Type | Update | Change |
|---|---|---|---|
| [vite](https://github.com/vitejs/vite) | devDependencies | patch | [\`5.4.14\` -> \`5.4.21\`](https://github.com/vitejs/vite/releases/tag/v5.4.21) |
`;

		const changes = parseRenovatePrBody(body);
		expect(changes).toEqual(['bump vite from 5.4.14 to 5.4.21']);
	});

	it('should parse 6-column Renovate PR tables', () => {
		const body = `
This PR contains the following updates:

| Package | Change | Age | Adoption | Passing | Confidence |
|---|---|---|---|---|---|
| [react](https://github.com/facebook/react) | \`18.2.0\` -> \`18.3.0\` | 3 days | 98% | 100% | high |
`;

		const changes = parseRenovatePrBody(body);
		expect(changes).toEqual(['bump react from 18.2.0 to 18.3.0']);
	});

	it('should fall back to PR title when no table is present in Renovate PR', () => {
		const body = `This PR was generated by Mend Renovate.`;
		const title = 'chore(deps): update actions/checkout to d23441a';

		const changes = parsePrBody(body, title);
		expect(changes).toEqual([
